                    ...
                    <tbody>
                        @foreach($data as $d)
                            <tr>
                                ...
                                <td>{{ $d->nis }}</td>
                                <td>{{ ucfirst($d->nama_siswa) }}</td>
                                <td>{{ $d->kelas }}</td>
                                <td>{{ strtoupper($d->jurusan) }}</td>
                                <td>
                                    <a href="{{ route('siswa.edit', $d->nis) }}" class="btn btn-sm btn-outline-success me-1" title="Edit">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>
                                    <form action="{{ route('siswa.delete', $d->nis) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
